...
try {
  $rawWorktree = & git grep -nEi --no-color -- $pattern -- . 2>$null
...
$rawHistory = & git -C $repoRoot log --all --pretty=format:%H --name-only --regexp-ignore-case --extended-regexp -G $pattern 2>$null
