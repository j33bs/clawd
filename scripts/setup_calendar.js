...
async function setupCalendarIntegration() {
  ...
  if (!config.calendarIntegration.enabled) {
    console.log("❌ Calendar integration is disabled in configuration");
    return;
  }
  ...
  console.log("\n🧪 Testing calendar integration setup...");
  
  // Attempt to initialize with empty credentials (will fail as expected)
  console.log("\n💡 To complete setup, you need to provide your calendar credentials.");
  ...
  console.log("node -e \"const CalendarIntegration = require('./calendar_integration.js'); const cal = new CalendarIntegration(); cal.initialize({username: process.env.CALENDAR_USERNAME, password: process.env.CALENDAR_PASSWORD}).then(() => console.log('✅ Success!')).catch(e => console.error('❌ Error:', e.message));\"");
...
initializeCalendar().catch(console.error);
  ...
  console.log("   • Automatically sync your calendar events every", config.calendarIntegration.syncInterval / 60000, "minutes");
  console.log("   • Provide availability summaries based on your real schedule");
  console.log("   • Store calendar data locally in", config.calendarIntegration.calendarFile);
  console.log("   • Detect busy periods and free time for planning");
...
if (require.main === module) {
  setupCalendarIntegration()
    .then(() => {
      console.log("\n✅ Calendar integration setup instructions complete!");
