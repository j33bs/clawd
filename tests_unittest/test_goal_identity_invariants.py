...
def _build_minimal_repo(root: Path, *, include_bypass_fixture: bool = False) -> None:
    ...
    # Minimal policy with required ladder and paid guards.
...
class TestGoalIdentityInvariants(unittest.TestCase):
    def test_verifier_passes_in_repo(self):
        ...
        verifier = host_repo_root / "workspace" / "scripts" / "verify_goal_identity_invariants.py"
    ...
    def test_verifier_strict_fails_on_fixture_warning(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            _build_minimal_repo(root, include_bypass_fixture=True)
