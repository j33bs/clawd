...
class TestPolicyRouterOpenAICompatibleDispatch(unittest.TestCase):
    ...
    def test_auth_handler_can_run_without_ready_env_and_use_override_model(self):
        policy_router = _load_policy_router_module()

        seen: dict[str, object] = {}

        def _handler(_payload, model_id, context):
            seen["model_id"] = model_id
            seen["exec_agent_id"] = context.get("exec_agent_id")
            return {"ok": True, "text": "ok"}
        ...
        policy = {
            ...
            "budgets": {
                "intents": {"conversation": {"dailyTokenBudget": 999999, "dailyCallBudget": 999, "maxCallsPerRun": 10}},
                "tiers": {"auth": {"dailyTokenBudget": 999999, "dailyCallBudget": 999}},
            },
            "providers": {
                "openai_auth": {
                    "enabled": True,
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            out = router.execute_with_escalation(
                ...
                {"prompt": "hi"},
                {
                    ...
                    "override_model": "openai/gpt-5.4-pro",
                    "exec_agent_id": "main",
                },
            )

        self.assertTrue(out.get("ok"), out)
        self.assertEqual(out.get("provider"), "openai_auth")
        self.assertEqual(out.get("model"), "openai/gpt-5.4-pro")
        self.assertEqual(seen.get("model_id"), "openai/gpt-5.4-pro")
        self.assertEqual(seen.get("exec_agent_id"), "main")
