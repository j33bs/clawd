...
function createAdapter(options = {}) {
  return async ({ profile, messages, metadata }) => {
    ...
    const result = await callModel({
      ...
      preferredBackend: profile === 'code_remote' ? BACKENDS.OATH_CLAUDE : null,
      metadata
    });
...
async function runChain(requestText, options = {}) {
  ...
  for (const task of state.plan.tasks) {
    ...
    await traceStep(traceOptions, {
      ...
      tokenEstIn: estimateTokens(messagesToText([{ content: exec.text }])),
    ...
    if (steps.critiqueNeeded(task, state.request.risk)) {
      ...
      await traceStep(traceOptions, {
        ...
        tokenEstIn: estimateTokens(task.title),
