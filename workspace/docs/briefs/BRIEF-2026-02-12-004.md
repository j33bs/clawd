...
File backend passphrase policy:
- Preferred: `SECRETS_FILE_PASSPHRASE_FILE=/secure/path/passphrase.txt` (permissions `0600`)
- `SECRETS_FILE_PASSPHRASE` is allowed only in `development`/`test` runtimes (or with `SECRETS_ALLOW_ENV_PASSPHRASE=1`)
