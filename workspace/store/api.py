...
INDEX_FILE  = os.path.join(WORKSPACE, "governance", "OPEN_QUESTIONS_INDEX.json")
...
@app.get("/index")
def get_index(x_store_key: Optional[str] = Header(default=None)):
    """
    Lightweight navigation index: section numbers, titles, authors, exec_tags,
    one-sentence synopses, and keyphrases. ~20 KB vs 451 KB for the raw doc.

    Protocol: governance sessions should start here, then fetch individual
    sections via GET /section/{n} when full body is needed.
    Generated by full_rebuild(); updated on every store rebuild.
    """
    if STORE_KEY:
        _require_key(x_store_key)
    if not os.path.exists(INDEX_FILE):
        raise HTTPException(
            status_code=404,
            detail="Index not yet generated — run /rebuild first.",
        )
    try:
