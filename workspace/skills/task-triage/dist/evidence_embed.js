...
function head(value, max = 400) {
  const text = String(value || "").trim();
  return text.length <= max ? text : text.slice(0, max);
}

function tokenize(text) {
  const words = String(text || "").toLowerCase().split(/[^a-z0-9_]+/).filter((w) => w.length >= 2);
...
function runProcess(cmd, args, stdinJson, timeoutMs) {
...
function coremlCliPath() {
  return path.join(__dirname, "..", "..", "coreml-embed", "dist", "cli.js");
}

function coremlRunnerPath() {
  return path.join(__dirname, "..", "..", "..", "runners", "coreml_embed_runner", "run.sh");
}

async function defaultCoremlHealth(modelPath, computeUnits, timeoutMs) {
  ...
  if (result.code !== 0) {
    const underlying = parsed && parsed.error && parsed.error.type ? String(parsed.error.type) : "HEALTH_FAILED";
...
async function defaultCoremlEmbed(modelPath, texts, computeUnits, timeoutMs) {
  ...
  const parsed = lastJsonLine(result.stdout);
  if (result.code !== 0) {
    if (parsed && parsed.error && parsed.error.type) {
...
function normalizeEvidenceConfig(rules) {
  const cfg = (rules && rules.evidence) || {};
  const cb = (cfg.coreml && cfg.coreml.circuit_breaker) || {};
  return {
    enabled: cfg.enabled !== false,
    strategy_preference: Array.isArray(cfg.strategy_preference) && cfg.strategy_preference.length > 0 ? cfg.strategy_preference : ["coreml_embed", "keyword_stub"],
    ...
    coreml: {
      enabled: (cfg.coreml && cfg.coreml.enabled) !== false,
      model_path: String((cfg.coreml && cfg.coreml.model_path) || ""),
      compute_units: String((cfg.coreml && cfg.coreml.compute_units) || "ALL"),
      max_texts_per_call: Number((cfg.coreml && cfg.coreml.max_texts_per_call) ?? 32),
      timeout_ms: Number((cfg.coreml && cfg.coreml.timeout_ms) ?? 30000),
      health_check: (cfg.coreml && cfg.coreml.health_check) !== false,
      circuit_breaker: {
        failure_window: Number(cb.failure_window ?? 10),
        max_failures: Number(cb.max_failures ?? 3),
        cooloff_ms: Number(cb.cooloff_ms ?? 60000),
      },
    },
    fallback_keyword_stub: {
      enabled: (cfg.fallback_keyword_stub && cfg.fallback_keyword_stub.enabled) !== false,
    },
  };
}

function chunkContext(context, chunkChars, maxContextChars) {
...
function selectKeyword(task, chunks, cfg, truncated, attempts, latencyMs) {
...
async function selectCoreml(task, chunks, cfg, truncated, attempts, deps) {
  const modelPath = String(cfg && cfg.coreml && cfg.coreml.model_path || "").trim();
  if (!modelPath) {
    throw makeError("EVIDENCE_EMBED_MODEL_NOT_CONFIGURED", "coreml model_path not configured", {});
  }

  const computeUnits = String(cfg && cfg.coreml && cfg.coreml.compute_units || "ALL");
  const timeoutMs = Number((cfg && cfg.coreml && cfg.coreml.timeout_ms) ?? 30000);
  const maxPerCall = Math.max(1, Math.floor(Number((cfg && cfg.coreml && cfg.coreml.max_texts_per_call) ?? 32)));
  const healthCheck = (cfg && cfg.coreml && cfg.coreml.health_check) !== false;

  const health = (deps && deps.coremlHealth) || defaultCoremlHealth;
  const embed = (deps && deps.coremlEmbed) || defaultCoremlEmbed;

  const cb = cfg && cfg.coreml && cfg.coreml.circuit_breaker || {};
  const maxFailures = Math.max(1, Math.floor(Number(cb.max_failures ?? 3)));
  let failures = 0;

  const tryOrThrow = async (fn) => {
    try {
      return await fn();
    } catch (err) {
      failures += 1;
      if (failures >= maxFailures) {
        const type = String(err && err.type || "EVIDENCE_EMBED_UNAVAILABLE");
        throw makeError(type, String(err && err.message || "coreml evidence failed"), {
          ...((err && err.details) || {}),
          circuit_open: true,
          failures,
...
async function selectEvidenceBundle(task, context, rules, deps) {
  ...
  for (const strategy of cfg.strategy_preference) {
    if (strategy === "coreml_embed" && cfg.coreml.enabled) {
      ...
      try {
        ...
        const errorType = String(err && err.type || "EVIDENCE_EMBED_UNAVAILABLE");
