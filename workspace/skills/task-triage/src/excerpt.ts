...
export function buildExcerpt(
  ...
  context: string,
  rules: any,
  sentinelMaxExcerptChars?: number
): ExcerptResult {
  const pref = rules?.prefilter || {};
...
export function buildEvidenceBundle(task: string, context: string, rules: any): {
  kind: "topk_stub";
  top_k: number;
  selected: Array<{ id: string; start: number; end: number; score: number }>;
  notes: string;
} | undefined {
  ...
  const selected: Array<{ id: string; start: number; end: number; score: number }> = [];
  const paragraphs = context.split(/\n{2,}/).filter((p) => p.trim().length > 0);
  let cursor = 0;
  let idx = 0;

  const pushChunk = (chunk: string, start: number, end: number) => {
