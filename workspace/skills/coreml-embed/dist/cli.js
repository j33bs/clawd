...
const VALID_COMPUTE_UNITS = ["ALL", "CPU_ONLY", "CPU_AND_GPU", "CPU_AND_NE"];
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    else if (key === "--compute_units") out.compute_units = String(val).toUpperCase();
...
function normalizeComputeUnits(value, fallback) {
...
function resolveRunnerTimeoutMs(cfg) {
...
function resolveMaxConcurrent(cfg) {
...
function cleanupStalePidFiles(dir, ttlMs) {
...
async function invokeRunner(args, inputJson, timeoutMs) {
  return new Promise((resolve) => {
    const proc = spawn("bash", [runnerScriptPath(), "--json", ...args], { stdio: ["pipe", "pipe", "pipe"] });
    let stdout = "";
    let stderr = "";
    let timedOut = false;
...
function validateInferenceInput(input, cfg) {
  ...
  const computeUnits = normalizeComputeUnits(input.compute_units, cfg.default_compute_units || "ALL");
...
async function runWithInput(input, deps = {}) {
  ...
  if (input.health) {
    ...
    if (healthRes.code !== 0) {
      if (healthPayload && healthPayload.ok === false && healthPayload.error && healthPayload.error.type) {
  ...
  try {
    ...
    if (runRes.code !== 0) {
      if (payload && payload.ok === false && payload.error && payload.error.type) {
...
async function run(argv = process.argv.slice(2)) {
  ...
  const merged = { ...stdinInput, ...argInput };
