...
TELEGRAM_MEMORY_STORE_LIMIT = 1200
TELEGRAM_MEMORY_LINE_LIMIT = 280
OPENCLAW_TRANSCRIPT_SOURCE = "openclaw_telegram_session"
...
def _memory_reject_reason(
    ...
    *,
    ...
    if any(marker in lowered for marker in BLOCKED_MEMORY_PATTERNS):
        return "filtered_noise"

    if str(source or "").strip().lower() == OPENCLAW_TRANSCRIPT_SOURCE:
        return "filtered_transcript_source"

    tool_dump_hits = sum(1 for marker in TOOL_DUMP_PATTERNS if marker in normalized)
    if tool_dump_hits >= 2:
        return "filtered_tool_dump"

    if normalized.startswith("System:") and ("exec denied" in lowered or "approval-timeout" in lowered):
        return "filtered_system_notice"
...
def _sanitize_content(
    ...
    *,
    ...
    normalized = _normalized_text(text)
    reject_reason = _memory_reject_reason(normalized, source=source, role=role)
    if reject_reason:
        return "", reject_reason
    return _compact(normalized, limit=TELEGRAM_MEMORY_STORE_LIMIT), ""
...
def _memory_line(row: dict[str, Any]) -> str:
    text = _compact(str(row.get("content") or ""), limit=TELEGRAM_MEMORY_LINE_LIMIT)
    if not text:
        return ""
...
def _normalize_role(value: Any) -> str:
    role = str(value or "participant").strip().lower() or "participant"
    if role in {"user", "assistant"}:
        return role
    return "participant"


def _recallable_row(row: dict[str, Any]) -> bool:
    role = _normalize_role(row.get("role"))
    if role not in {"user", "assistant"}:
        return False
    content = _normalized_text(str(row.get("content") or ""))
    if not content:
        return False
    if _memory_reject_reason(content, source=str(row.get("source") or ""), role=role):
        return False
    if len(content) <= 360:
        return True
    return role == "assistant" and _looks_like_commitment(content)


def normalize_telegram_memory_row(row: dict[str, Any]) -> dict[str, Any] | None:
    ...
    content, reject_reason = _sanitize_content(
        str(row.get("content") or ""),
        source=str(row.get("source") or ""),
        role=role,
    )
    if reject_reason or not content:
        return None

    normalized = dict(row)
    normalized["chat_id"] = str(row.get("chat_id") or "").strip()
    normalized["chat_title"] = str(row.get("chat_title") or "unknown").strip() or "unknown"
    normalized["message_id"] = str(row.get("message_id") or "").strip()
    normalized["author_id"] = str(row.get("author_id")) if row.get("author_id") is not None else None
    normalized["author_name"] = str(row.get("author_name") or "unknown").strip() or "unknown"
    normalized["role"] = role
    normalized["content"] = content
    normalized["agent_scope"] = str(row.get("agent_scope") or "main").strip() or "main"
    normalized["source"] = str(row.get("source") or "telegram").strip() or "telegram"
    normalized["meta"] = dict(row.get("meta") or {})
    normalized["stored_at"] = str(row.get("stored_at") or _now_iso())
    normalized["created_at"] = str(row.get("created_at") or normalized["stored_at"])
    return normalized


def sanitize_telegram_memory_archive(
    *,
    path: Path | None = None,
    state_path: Path | None = None,
) -> dict[str, Any]:
    target_path = path or TELEGRAM_MEMORY_PATH
    target_state_path = state_path or TELEGRAM_MEMORY_STATE_PATH
    rows = _load_rows(target_path)
    ...
    for row in rows:
        normalized = normalize_telegram_memory_row(row)
        if normalized is None:
            filtered += 1
            continue
        memory_key = _memory_key(normalized.get("chat_id", ""), normalized.get("message_id", ""))
    ...
    with target_path.open("w", encoding="utf-8") as handle:
        for row in kept:
            handle.write(json.dumps(row, ensure_ascii=False) + "\n")
    _save_state(target_state_path, seen_keys)
    return {
        "ok": True,
...
def ingest_telegram_exchange(
    *,
    ...
    normalized_content, reject_reason = _sanitize_content(
        str(content or ""),
        source=str(source or "telegram"),
        role=str(role or "participant"),
    )
    if reject_reason:
        return {"stored": False, "reason": reject_reason}
...
def build_telegram_memory_context(
    *,
    ...
    selected_assistant: list[str] = []
    selected_user: list[str] = []
    seen: set[str] = set()
    ...
    for row in reversed(rows):
        ...
        if row_chat_id != normalized_chat_id:
            continue
        if not _recallable_row(row):
            continue
        ...
        elif row_role == "assistant":
            selected_assistant.append(line)
        else:
            selected_user.append(line)

        if len(selected_thread) + len(selected_assistant) + len(selected_user) >= max(1, int(limit)) * 3:
            break

    merged = selected_thread[: max(1, int(limit))]
    if len(merged) < max(1, int(limit)):
        merged.extend(selected_assistant[: max(1, int(limit)) - len(merged)])
    if len(merged) < max(1, int(limit)):
        merged.extend(selected_user[: max(1, int(limit)) - len(merged)])
